...
QUESTION_CASES = [
    {
        "question": "Qwen3-Next는 어떤 기술로 효율성을 높였나요?",
        "sparse_query": "Qwen3-Next OR 효율성 OR 훈련 OR 추론",
    },
    {
        "question": "교육 분야에서 AI는 어떻게 활용되고 있나요?",
        "sparse_query": "교육 OR AI OR 활용",
    },
]
...
def main():
    ...
    for case in QUESTION_CASES:
        ...
        print("\n[DENSE 결과]")
        dense_results = dense_search(conn, question, top_k=top_k)
        print_results(dense_results)

        print("\n[SPARSE 결과]")
        print("키워드:", sparse_query)
        sparse_results = sparse_search(conn, sparse_query, top_k=top_k)
        print_results(sparse_results)
