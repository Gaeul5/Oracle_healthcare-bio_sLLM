"""
R2/R3/R4 교사 데이터 생성 (Qwen2.5-7B-Instruct, 4bit) — HF transformers 순차 생성 버전
=====================================================================
generate_teacher_data_vllm.py(배치 추론, T4 처리량 미검증) 도입 시 만든 백업.
문서당 실측 32.8초로 검증된 안전한 경로 — vLLM go/no-go 판단에서 no-go가
나오거나 문제가 생기면 이 파일로 되돌아올 것 (generate_teacher_data.py와 동일 내용).
...
def generate_one(tok, model, prompt_template, text):
    ...
    out = model.generate(**inputs, pad_token_id=tok.pad_token_id, **GEN_KWARGS)
