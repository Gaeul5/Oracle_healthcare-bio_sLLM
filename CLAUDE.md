...
1. `build_unified.py` 실행 → `data/unified.jsonl` 생성 (현재 없음, gitignore 대상이라
   로컬에서만 생성하고 커밋하지 않음)
2. `Qwen2.5-7B-Instruct` 4bit로 R2/R3/R4 데이터 생성 스크립트 작성 (Colab, 중간 저장 포함)
   → `check_length_balance()`로 토큰 수 ±20% 정렬 검증
3. `Qwen2.5-0.5B/1.5B/3B-Instruct`로 QLoRA 학습, R0~R4 각 조건 (약물 기준 그룹
   교차검증 폴드 사용, 별도 Colab 세션)
4. `evaluate.py`로 도메인 간(PHEE↔CADEC+PsyTAR) strict/relaxed F1 측정
5. 폴드 간 표준편차 vs 시드 간 표준편차 비교 (supplementary figure로 남길 것 — 이미
