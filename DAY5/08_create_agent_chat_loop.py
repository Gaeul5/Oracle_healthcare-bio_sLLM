...
def list_collections() -> str:
    """PGVector에 저장된 collection 목록을 반환합니다. collection 상태나 문서 저장 여부를 확인할 때 사용합니다."""
    sql = """
        SELECT name FROM langchain_pg_collection ORDER BY name;
    """
    with psycopg.connect(**get_db_config()) as conn:
        with conn.cursor() as cur:
            cur.execute(sql)
...
print("=" * 80)
print("08. create_agent 채팅 루프")
print("여러 질문을 입력하면서 Tool 선택을 비교합니다.")
...
while True:
    ...
    # 이 파일은 질문별 Tool 선택을 비교하기 위해 각 질문을 독립적으로 실행합니다.
    result = agent.invoke({"messages": [{"role": "user", "content": user_input}]})
    used_tool_names = get_used_tool_sequence(result["messages"])

    print("\n[최종 답변]")
    print(f"사용한 Tool: {', '.join(used_tool_names) if used_tool_names else '없음'}")
